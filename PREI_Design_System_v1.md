...
| Tespit | Sorun | Karar |
|---|---|---|
| `--color-primary-purple: #9B5BB3` | Jenerik "AI moru" — marka kimliği yok | ❌ Tamamen çıkar |
...
3. **Renk = anlam.** Accent dekorasyon değil; dikkat yönetimi aracı. Yeşil sadece pozitif, kırmızı sadece negatif/risk, şampanya sadece marka ve birincil aksiyon.
...
:root {
  /* ===== Marka ===== */
  --brand-champagne:      #C6A15B;   /* birincil accent — CTA, aktif durum, marka vurgusu */
  --brand-champagne-dim:  #A88A4F;   /* hover/pressed */
  --brand-champagne-soft: rgba(198, 161, 91, 0.12); /* seçili arkaplan, chip */
  ...
  --chart-1: #C6A15B;  --chart-2: #5B8DB8;  --chart-3: #3FA372;  --chart-4: #8B7CB8;
...
[data-theme='dark'] {
  ...
  --border-focus:   var(--brand-champagne);
...
[data-theme='light'] {
  ...
  --border-focus:   var(--brand-champagne-dim);
...
- Eski `--color-primary-purple` vb. token'lar bir geçiş haritasıyla (`purple → champagne`) alias'lanır, modüller taşındıkça alias'lar silinir.
...
### 5.4 Butonlar
- **Primary:** `--brand-champagne` zemin, koyu metin (#1B1A18) — kontrast AA garantili. Sayfada tek primary.
...
- **Renk:** tek serili grafik = `--chart-1` (şampanya); karşılaştırma = rampa sırayla; pozitif/negatif ayrımı olan her seri semantik renkleri kullanır.
...
- [ ] `#9B5BB3` ve türevleri kod tabanında sıfır geçiyor; tek accent = şampanya, sayfa genelinde kilitli.
